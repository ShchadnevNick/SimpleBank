...
// Отображение транзакций и Сортировка
const displayTransactions = function (account, sort = false) {
  //innerHTML похож на textContent, но textContent возвращает текст, а innerHTML возвращает также и HTML код.
  // const containerTransactions = document.querySelector('.transactions');
  containerTransactions.innerHTML = '';
  // slice исползуется для создания массива, т.к. sort меняет исходник
  const transacs = sort
  ...
  transacs.forEach(function (trans, index) {
    ...
    // insertAdjacentHTML позволяет вставить строку HTML кода в любое место страницы. Код вставляется относительно опорного элемента.
    // beforeBegin вставку перед опорным элементом
    // afterEnd вставка после опорного элемента
    // afterBegin вставка в начало опорного элемента
    // beforeEnd в конец опорного элемента.
    containerTransactions.insertAdjacentHTML('afterbegin', transactionRow);
  });
};
//Создаем логины пользователей для входа
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    //Display UI(user interface) and welcome message
    containerApp.style.opacity = 1;
    ...
    inputLoginPin.value = '';
    //No focus Вызов метода blur() на DOM-элементе снимает фокус с элемента
    inputLoginPin.blur();
    ...
    //Display transaction,balance,total
    updateUi(currentAccount);
  }
});
